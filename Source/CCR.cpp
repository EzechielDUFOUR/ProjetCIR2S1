		...
		for (auto p : FlyingPlanes_) {
			...
			for (auto p2 : FlyingPlanes_) {
				if (p != p2) {
					...
					pFuture.x = p->getPos().x + 5*p->getTrajectory().x * p->getSpeed() / 3600; pFuture.y = p->getPos().y + 5*p->getTrajectory().y * p->getSpeed() / 3600;
					p2Future.x = p2->getPos().x + 5*p2->getTrajectory().x * p2->getSpeed() / 3600; p2Future.y = p2->getPos().y + 5*p2->getTrajectory().y * p2->getSpeed() / 3600;
					
					double dx = pFuture.x - p2Future.x, dy = pFuture.y - p2Future.y;
					double dist = sqrt(dx * dx + dy * dy);
					if (dist <= 20.0 && (p->getState()!=EVASION || p2->getState()!=EVASION) && abs(p->getPos().altitude-p2->getPos().altitude)<=0.1) {
					...
					else if (sqrt((p->getPos().x - p2->getPos().x) * (p->getPos().x - p2->getPos().x) + (p->getPos().y - p2->getPos().y) * (p->getPos().y - p2->getPos().y)) >= 20 && p->getState()==EVASION && p2->getState()==EVASION ){
